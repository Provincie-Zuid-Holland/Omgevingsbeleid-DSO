...
class PolicyObjectRepository:
    ...
    def get_optional(self, object_type: str, object_id: int) -> PolicyObject | None:
        code: str = f"{object_type}-{object_id}"
        result: PolicyObject | None = self._data.get(code, None)
        return result

    def get(self, object_type: str, object_id: int) -> PolicyObject:
        o: PolicyObject | None = self.get_optional(object_type, object_id)
        if o is None:
            raise RuntimeError(f"Can not find object {object_type}-{object_id}")
        return o

    def get_by_code(self, object_code: str) -> PolicyObject:
        o: PolicyObject | None = self._data.get(object_code, None)
        if o is None:
